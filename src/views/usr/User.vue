<template>
  <div>
    ...
    <!--编辑对话框，可完成修改功能-->
    <el-dialog title="学生信息" :visible.sync="dialogFormVisible" width="600px" @closed="resetForm('editForm')">
      <!--:model用于绑定数据，:rules用于验证规则，ref可用于表单验证-->
...
export default {
  ...
  methods:{
    ...
    getList(){
      ...
      this.$axios.get('/api/user/select',{params:params}).then(res=>{
  ...
  //获取对应数据填充到editForm中
    editHandle(username){     
      ...
      console.log(params)
      this.$axios.get('/api/user/select_username',{params:params}).then(res=>{
    ...
    submitEditForm(formName){      
      this.$refs[formName].validate((valid)=>{            //验证表单是否符合标准
        if(valid){
          this.$axios.post('/api/user/'+(this.addOrUpdate?"add":"update"),this.editForm).then(res=>{
    ...
    delHandle(id){
      ...
      this.$axios.get('/api/user/delete',{params:params}).then(res=>{
        if(res.data.status==200){
          this.getList() 
          //如果删除第一条数据，则需跳到前一页

...
//按姓名查询
router.get('/select_username',function(req,res,next){
    //params只有username的信息
...
router.post('/add',function(req,res,next){
    ...
    console.log(params)
    mssqldb.querySql('insert into '+tableName+'(username,password) values(@username,@password)',{username:params.username,password:params.password},function(err,result){
        let response ={
            status:200
        }
        if(err){
...
router.post('/update',function(req,res,next){
    const params =req.body
    console.log(params)
    mssqldb.update({username : params.username,password: params.password},{id:params.id},tableName,function(err,result){
        let response ={
            status:200,
        }
        if(err){
